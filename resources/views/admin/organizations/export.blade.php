    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            ...
            <form action="{{ route('admin.organizations.export') }}" method="GET">
                @if(request('org_ids'))
                    <input type="hidden" name="org_ids" value="{{ request('org_ids') }}">

                    <div class="mb-6 p-3 bg-indigo-50 text-indigo-700 rounded-lg text-sm flex items-center">
                        <i class="fas fa-check-circle mr-2"></i>
                        Đang xuất dữ liệu cho {{ count(explode(',', request('org_ids'))) }} tổ chức đã chọn.
                    </div>
                @endif
                <!-- Filters -->
