                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" name="end_date" min="{{ date('Y-m-d', strtotime('+3 days')) }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                            </div>
...
@push('scripts')
    ...
    <script>
        ...
        // Set min date for Start Date to today (optional)
        const today = new Date().toISOString().split('T')[0];
        startDateInput.setAttribute('min', today);

        // Khi thay đổi Start Date
        startDateInput.addEventListener('change', function () {
            ...
            if (endDateInput.value && endDateInput.value < startDateVal) {
                endDateInput.value = '';
                alert('End date cannot be earlier than start date. Please select a new end date.');
            }
        });

        // Khi thay đổi End Date
...
@push('styles')
    ...
    <style>
        ...
        .ts-control {
            ...
            border-color: #d1d5db;
            min-height: 42px;
            /* Đảm bảo chiều cao không bị nhảy */
            background-image: none !important;
            /* Xóa icon mũi tên mặc định nếu có */
        }
