            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex items-start gap-6">
                        <img src="{{ $volunteer->avatar_url ? asset('storage/' . $volunteer->avatar_url) : 'https://ui-avatars.com/api/?name=' . urlencode($volunteer->first_name . ' ' . $volunteer->last_name) }}"
                            alt="{{ $volunteer->first_name }}" class="w-24 h-24 rounded-full object-cover">
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-4 text-sm text-gray-600">
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="mailto:{{ $volunteer->email }}"
                                        class="hover:text-green-600">{{ $volunteer->email }}</a>
                                </div>
                                @if($volunteer->phone)
                                    <div class="flex items-center gap-2">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @if($volunteer->volunteerProfile)
                                <div class="flex items-center gap-3 mt-4">
                                    ...
                                    <span class="text-gray-600">({{ $volunteer->volunteerProfile->total_volunteer_hours ?? 0 }}
                                        hours)</span>
                                </div>
                            @endif
                        </div>
                ...
                @if($volunteer->volunteerProfile && $volunteer->volunteerProfile->volunteer_experience)
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <p class="text-gray-700 whitespace-pre-line">{{ $volunteer->volunteerProfile->volunteer_experience }}
                        </p>
                    </div>
                @endif
                            ...
                            @foreach($applications as $application)
                                <div
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">Applied {{ $application->applied_date->format('M d, Y') }}
                                        </p>
                                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <button onclick="confirmKickOnProfile({{ $volunteer->user_id }})"
                            class="block w-full px-4 py-3 text-center border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition mt-2">
...
@push('scripts')
    <script>
        ...
        function confirmKickOnProfile(volunteerId) {
            if (!confirm('Are you sure you want to remove this volunteer from your organization? This action cannot be undone.')) {
                return;
            }

            fetch(`/organization/volunteers/${volunteerId}/remove`, {
                method: 'DELETE',
                headers: {
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        window.location.href = '{{ route("organization.volunteers.index") }}'; // Quay về danh sách
                    } else {
                        alert(result.message);
                    }
                })
