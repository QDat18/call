        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ number_format($stats['total_hours']) }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $stats['disputed'] }}</p>
                </div>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <img src="{{ $activity->volunteer->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($activity->volunteer->first_name) }}" 
...
document.getElementById('verifyForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.message || 'Failed to verify activity');
        }
    })
    .catch(error => {
...
document.getElementById('disputeForm').addEventListener('submit', function(e) {
    ...
    fetch(`/organization/activities/${currentActivityId}/dispute`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            'Content-Type': 'application/json',
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.message || 'Failed to dispute activity');
        }
    })
    .catch(error => {
        console.error('Error:', error);
